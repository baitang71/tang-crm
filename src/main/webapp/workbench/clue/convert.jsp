...
<script type="text/javascript">
	$(function(){
		...
		$("#selectByName").keydown(function (event) {
			if (event.keyCode==13){
				var aname=$("#selectByName").val()
				$.ajax({
		...
		$("#commitBtn").click(function (){
			var $cb=$("input[name=activity]:checked")
			if ($cb.length==0){
				alert("请勾选市场活动")
			}else{
				var activityId=$cb.val()
				var activityName=$("#t"+activityId).html()
		...
		$("#tranBtn").click(function (){
			if ($("#isCreateTransaction").prop("checked")){
				...
				window.location.href="workbench/clue/tran.do?clueId="+"${param.clueId}"
			}
		})
	});
</script>
...
<body>
					...
					<div class="btn-group" style="position: relative; top: 18%; left: 8px;">
						<form class="form-inline" role="form">
						  <div class="form-group has-feedback">
						    <input type="text" id="selectByName" class="form-control" style="width: 300px;" placeholder="请输入市场活动名称，支持模糊查询">
	...
	<div id="create-transaction2" style="position: relative; left: 40px; top: 20px; width: 80%; background-color: #F7F7F7; display: none;" >
		...
		<form action="workbench/clue/tran.do" method="post" id="tranForm">
